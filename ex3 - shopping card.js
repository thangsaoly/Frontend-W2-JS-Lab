...
function addProductToCart(productId) {
  // Write your code here
  let x = SHOPPING_CART.find((item) => item.id === productId);
  if (x) {
    x.quantity++;
  } else {
 ...
 *
 *  - CASE 3 : if the product id does NOT exist : do nothing !
...
function removeProductFromCart(productId) {
  ...
  if (!x) return; // Product not in cart, do nothing

  if (x.quantity >= 2) {
    x.quantity--; // If quantity >= 2 we subtract 1
  } else { // if the quantity is 1 we remove the obj from the cart
    let index = SHOPPING_CART.findIndex((item) => item === x);
